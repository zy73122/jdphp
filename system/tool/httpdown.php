...
/**
 * 使用示例：
include_once(PATH_TOOL.'httpdown.php');
//下载图片
echo GetRemoteImage("http://www.qvodzy.com.cn/UpFiles/2009042353177461.jpg");
//下载flash
echo GetRemoteFlash("http://k.kbcdn.com/images/weather/swf/w3.swf"); 
 */

define ('HTTPDOWN_DIR_IMAGE', "data/httpdown/image");
define ('HTTPDOWN_DIR_MEDIA', "data/httpdown/media");
...
function GetRemoteImage($url,$uid=0)
{
	...
	{
		$imgUrl = HTTPDOWN_DIR_IMAGE."/".date("ymd",time());
		$imgPath = PATH_ROOT.$imgUrl;
		cfile::mkdir_recursive($imgPath, 0700);
		...
		do
		{
			$rndname = date("His",time()).mt_rand(1000,9999);
		}
		while(file_exists($rndname));
...
function GetRemoteFlash($url,$uid=0)
{
	...
	{
		$imgUrl = HTTPDOWN_DIR_MEDIA."/".date("ymd",time());
		$imgPath = PATH_ROOT.$imgUrl;
		cfile::mkdir_recursive($imgPath, 0700);
		$itype = ".swf";
		do
		...
		while(file_exists($rndname));
		$rndFileName = $imgPath."/".$rndname.$itype;
		$ok = $htd->SaveToBin($rndFileName);
		if($ok)
		{
			$revalues = $rndFileName;
		}
	}
	$htd->Close();
	return $revalues;
