...
class update
{
	 ...
	 */
	 public static function check_update()
	 {
		try
		{
			$uptime = trim( file_get_contents( PATH_DATA . self::$update_dir . '/lastupdate.txt' ) );
			$verlist = downloader::get_data( self::$source_url . self::$current_version . '/update-list.txt' );
			if(!$verlist || $uptime=='')
			{
				return false;
			}
	 ...
	 * @throws none
	 */
	 public static function getfilelist($upitems, $tmpdir)
	 {
		try
		{
			...
			foreach($upitemsArr as $upitem)
			{
				...
				$filelist = downloader::get_data($durl);
				$filelist = $filelist;
   ...
   public static function write_cache($tmpdir, $lasttime, $upitems, $files, $skipnodir)
   {
		...
		foreach($files as $filename)
		{
			...
			$curdir = self::get_dir_name($tfilename);
