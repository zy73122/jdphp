...
/*
 * [UAP Server SDK]
 * UAP Server SDK HTTP请求类
 * $Id: Request.class.php 43738 2009-08-12 07:26:03Z chenguo $
*/
...
class UapRequest {
	...
	public function sendRequest() {
		...
		$this->_response = new UapRestResponse();
		if (empty($result))
		{
			$this->_response->setResponseCode('500');
			$this->_response->setResponseBody('uap internal error');
		}
		else
		{
			preg_match('|HTTP/1\.1\s+(\d+)|', $result['header'], $m);
			$this->_response->setResponseCode($m[1]);
			$this->_response->setResponseBody($result['data']);
		}
	...
	private function _uc_fopen($limit = 0, $ip = '', $timeout = 15, $block = TRUE) {
		...
		if($this->getSid() && !$this->getApikey()) {
			if($this->getQueryString()) {
				$matches['query'] = 'sid='.$this->getSid().'&'.$this->getQueryString();
			} else {
				$matches['query'] = 'sid='.$this->getSid();
			}
		} else {
			if (strpos($type, 'login') === FALSE)
				$matches['query'] = ($this->getQueryString()) ? "apikey=".$this->getApikey().'&'.$this->getQueryString() :"apikey=".$this->getApikey();
		}
		$host = $matches['host'];
